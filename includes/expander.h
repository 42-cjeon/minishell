...
/*   Updated: 2022/01/26 17:38:59 by hanelee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				get_wildcard_strs(t_expander_context *context, \
						t_wildcard_info *wcinfo);
void				get_wildcard_info(t_expander_context *context, \
						t_wildcard_info *wcinfo);
...
void				append_last_token(t_wildcard_info *wcinfo, \
						t_str_node *last);
t_bool				match_part(t_str_node *str_node, \
						const char *filename, size_t filename_len, \
						size_t left_char);
t_bool				match_filename(t_wildcard_info *wcinfo, \
						const char *filename);
int					append_matched_for_each(DIR *ent, \
						t_wildcard_info *wcinfo, \
						t_tokenv *filenames);
int					append_matched(t_wildcard_info *wcinfo, \
						t_tokenv *filenames);
void				append_nomatch(t_wildcard_info *wcinfo, t_tokenv *tokenv);
t_bool				exp_is_wildcard_start(t_token_node *curr, \
						t_token_node *next);
...
size_t				replace_variable(t_shell_info *si, \
						char **str, const size_t start);
