...
/*   Updated: 2022/01/28 10:18:40 by cjeon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		str_node_push(t_wildcard_info *wcinfo, t_str_node **ptail, \
							t_str_node *node);
void		wildcard_info_clear(t_wildcard_info *wcinfo);
void		add_first_token(t_wildcard_info *wcinfo, const char *path);
void		wildcard_push_last(t_wildcard_info *wcinfo, t_str_node **tail, \
								t_str_node **last);
...
int			for_each_node(t_shell_info *si, t_tokenv *tokenv, \
							t_expander expander);
size_t		replace_variable(t_shell_info *si, char **str, const size_t start);
size_t		get_varname_len(char *str);
char		*get_var_from_env(t_envs *envs, char *str, size_t start, \
								size_t len);
#endif